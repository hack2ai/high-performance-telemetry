...
struct SummaryResult {
    BenchmarkResult representative{};
...
void print_usage(const char* program) {
    std::cout << "Usage: " << program << " [options]\n\nOptions:\n"
              << "  --packets N    Number of synthetic packets per run (default: 100000)\n"
              << "  --payload N    Payload size in bytes, 1-1024 (default: 128)\n"
              << "  --warmup N     Unreported warm-up runs before measurement (default: 1)\n"
...
Config parse_args(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--packets" || arg == "--payload" || arg == "--warmup" || arg == "--runs" || arg == "--csv") {
            if (i + 1 >= argc) throw std::invalid_argument("Missing value for " + arg);
            if (arg == "--csv") config.csv_path = argv[++i];
            else if (arg == "--packets") config.packets = parse_size(argv[++i], arg.c_str());
            else if (arg == "--payload") config.payload_size = parse_size(argv[++i], arg.c_str());
            else if (arg == "--warmup") config.warmup_runs = parse_size(argv[++i], arg.c_str());
            else config.measured_runs = parse_size(argv[++i], arg.c_str());
            continue;
        }
    ...
    if (config.measured_runs == 0) throw std::invalid_argument("--runs must be greater than 0");
...
BenchmarkResult run_benchmark(std::size_t packets, std::size_t payload_size) {
    ...
    benchmark.integrity_errors = integrity_errors.load(std::memory_order_relaxed);
    benchmark.ordering_errors = ordering_errors.load(std::memory_order_relaxed);
...
SummaryResult measure_repeated(const Config& config, std::size_t payload_size) {
    for (std::size_t i = 0; i < config.warmup_runs; ++i) run_benchmark(config.packets, payload_size);

    std::vector<double> throughputs;
    throughputs.reserve(config.measured_runs);
    SummaryResult summary;
    for (std::size_t i = 0; i < config.measured_runs; ++i) {
        const auto result = run_benchmark(config.packets, payload_size);
        if (result.processed != config.packets || result.integrity_errors != 0 || result.ordering_errors != 0) {
            throw std::runtime_error("Benchmark validation failed for payload " + std::to_string(payload_size) + " bytes");
        }
        throughputs.push_back(result.throughput);
        if (i == 0 || result.throughput > summary.representative.throughput) summary.representative = result;
        summary.min_throughput = std::min(summary.min_throughput, result.throughput);
        summary.max_throughput = std::max(summary.max_throughput, result.throughput);
    }

    for (const double value : throughputs) summary.mean_throughput += value;
    summary.mean_throughput /= static_cast<double>(throughputs.size());
    double variance = 0.0;
    for (const double value : throughputs) {
        const double delta = value - summary.mean_throughput;
        variance += delta * delta;
    }
    summary.stdev_throughput = throughputs.size() > 1 ? std::sqrt(variance / static_cast<double>(throughputs.size() - 1)) : 0.0;
    return summary;
}

void print_single_result(const Config& config, const SummaryResult& summary) {
    const auto& r = summary.representative;
    std::cout << "================================================\nHIGH-PERFORMANCE REAL-TIME TELEMETRY\n================================================\n"
              << "Packets/run    : " << config.packets << "\nPayload size   : " << config.payload_size << " bytes\n"
              ...
              << "Representative throughput : " << std::fixed << std::setprecision(2) << r.throughput << " packets/s\n"
              << "Mean throughput          : " << summary.mean_throughput << " packets/s\n"
              << "Min/Max throughput       : " << summary.min_throughput << " / " << summary.max_throughput << " packets/s\n"
              << "Throughput stdev         : " << summary.stdev_throughput << " packets/s\n"
              << "Avg latency     : " << std::setprecision(3) << r.avg_latency_us << " us\nP50 latency     : " << r.p50_latency_us << " us\n"
...
void print_matrix(const Config& config) {
    ...
    if (!config.csv_path.empty()) {
        ...
        csv << "payload_bytes,packets,warmup_runs,measured_runs,mean_throughput_packets_per_sec,min_throughput_packets_per_sec,max_throughput_packets_per_sec,throughput_stdev,representative_avg_latency_us,representative_p50_latency_us,representative_p95_latency_us,representative_p99_latency_us,full_retries,peak_queue,queue_memory_bytes,integrity_errors,ordering_errors\n";
    }
    std::cout << "================================================================================\nTELEMETRY BENCHMARK MATRIX\n================================================================================\nPackets per run: " << config.packets << " | Warm-up: " << config.warmup_runs << " | Measured: " << config.measured_runs << "\n\n";
    std::cout << std::left << std::setw(10) << "Payload" << std::setw(18) << "Mean p/s" << std::setw(18) << "Min p/s" << std::setw(18) << "Max p/s" << std::setw(14) << "Stdev" << std::setw(14) << "P95(us)" << std::setw(14) << "Peak Queue" << '\n';
    for (const auto payload_size : kBenchmarkPayloads) {
        const auto summary = measure_repeated(config, payload_size);
        const auto& r = summary.representative;
        std::cout << std::left << std::setw(10) << payload_size << std::setw(18) << std::fixed << std::setprecision(2) << summary.mean_throughput
                  << std::setw(18) << summary.min_throughput << std::setw(18) << summary.max_throughput << std::setw(14) << summary.stdev_throughput
                  << std::setw(14) << std::setprecision(3) << r.p95_latency_us << std::setw(14) << r.peak_queue_depth << '\n';
        if (csv) csv << payload_size << ',' << config.packets << ',' << config.warmup_runs << ',' << config.measured_runs << ','
                     << std::fixed << std::setprecision(3) << summary.mean_throughput << ',' << summary.min_throughput << ',' << summary.max_throughput << ','
                     << summary.stdev_throughput << ',' << r.avg_latency_us << ',' << r.p50_latency_us << ',' << r.p95_latency_us << ',' << r.p99_latency_us << ','
                     << r.full_retries << ',' << r.peak_queue_depth << ',' << r.queue_memory_bytes << ',' << r.integrity_errors << ',' << r.ordering_errors << '\n';
    }
    std::cout << "================================================================================\n";
...
int main(int argc, char* argv[]) {
    try {
        ...
        if (config.matrix) {
            telemetry::log(telemetry::LogLevel::info, "benchmark_matrix", "running repeated payload-size matrix");
            print_matrix(config);
            telemetry::log(telemetry::LogLevel::info, "shutdown", "benchmark completed");
            return 0;
        }
        const auto summary = measure_repeated(config, config.payload_size);
        print_single_result(config, summary);
        telemetry::log(summary.representative.integrity_errors == 0 && summary.representative.ordering_errors == 0 ? telemetry::LogLevel::info : telemetry::LogLevel::error,
                        "benchmark_result", "validation passed");
        return 0;
    } catch (const std::exception& error) {
        telemetry::log(telemetry::LogLevel::error, "exception", error.what());
