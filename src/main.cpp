...
Config parse_args(int argc, char* argv[]) {
    ...
    if (config.packets == 0) {
        throw std::invalid_argument("--packets must be greater than 0");
    }
...
int main(int argc, char* argv[]) {
    try {
        ...
        std::cout << "Queue storage  : " << (queue.capacity() * sizeof(telemetry::Frame))
                  << " bytes\n\n";
        ...
        return result.processed == config.packets ? 0 : 1;
