...
int main() {
    ...
    assert(!queue.try_push(1, 2, nullptr, payload.size(), 0));
    assert(!queue.try_push(1, 2, payload.data(), telemetry::kMaxPayloadSize + 1, 0));

    for (std::size_t i = 0; i < queue.capacity(); ++i) {
        assert(queue.try_push(10, 20, payload.data(), payload.size(), 0x01));
    }
    ...
    std::thread producer([&] {
        ...
        for (std::size_t i = 0; i < iterations; ++i) {
            ...
            while (!concurrent_queue.try_push(1, 2, data.data(), data.size(), 0)) {
    ...
    std::thread consumer([&] {
        ...
        while (count < iterations) {
            ...
            if (out.header.payload_len != 8 || out.header.source != 1 ||
